...
The script calculate the TOTAL value but without the 
6 MONTH MOVING AVERAGE that is defined by Total from the start of the nmme 1991 based on 
Brodie et al., [2023]
...
if __name__ == "__main__":
    ...
    for nmodel,model in enumerate(model_use_list):
        if model in avai_model_list:
            ...
            # CLOSE the 6-month rolling mean of the previous six month
            # da_total = da_ssta_areamean.rolling(L=6, center=False).mean()
            da_total = da_ssta_areamean
            ...
            # print('calculating TOTAL 6 month rolling closed')
            # # identify the event
    ...
    if ENS_OUTPUT:
        ...
        NOTES = 'TOTAL SSTA for all ensemble member derived from '
